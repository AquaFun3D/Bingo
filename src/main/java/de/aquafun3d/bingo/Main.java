...
import de.aquafun3d.bingo.utils.inventories.SettingsInventory;
...
public final class Main extends JavaPlugin {
	...
	public void onEnable() {
		...
		var teamSelectInv = new TeamselectInventory(helpers);
		var settingsInv = new SettingsInventory(helpers);
		var cage = new SpawnCage();
		var timer = new Timer(this,helpers);

		commandRegistration(helpers,config,teams);
		listenerRegistration(helpers,scoreboards,teams,teamSelectInv,config,teamSelectInv,settingsInv);
	...
	private void listenerRegistration(IHelpers helpers, IScoreboards scoreboards, ITeams teams, IInventory teamSelectInv, IConfig config, IInventory teamselect, IInventory settings) {
		PluginManager pluginManager = Bukkit.getPluginManager();
		pluginManager.registerEvents(new DefaultListener(helpers, scoreboards,settings,teamselect,teams),this);
